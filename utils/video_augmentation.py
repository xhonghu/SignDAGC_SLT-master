...
def clean_phoenix_2014_trans(prediction):
    ...
    # 删除特定标记
    prediction = re.sub(r"__LEFTHAND__", "", prediction, flags=re.IGNORECASE)
    prediction = re.sub(r"__EPENTHESIS__", "", prediction, flags=re.IGNORECASE)
    prediction = re.sub(r"__EMOTION__", "", prediction, flags=re.IGNORECASE)
    prediction = re.sub(r"\b__[^_ ]*__\b", "", prediction, flags=re.IGNORECASE)
    
    # 替换特定前缀或后缀
    prediction = re.sub(r"\bloc-([^ ]*)\b", r"\1", prediction, flags=re.IGNORECASE)
    prediction = re.sub(r"\bcl-([^ ]*)\b", r"\1", prediction, flags=re.IGNORECASE)
    prediction = re.sub(r"\b([^ ]*)-plusplus\b", r"\1", prediction, flags=re.IGNORECASE)
    prediction = re.sub(r"\b([a-zA-Z][a-zA-Z]*)raum\b", r"\1", prediction, flags=re.IGNORECASE)
    
    # 特定替换：连接单词
    prediction = re.sub(r"wie aussehen", "wie-aussehen", prediction, flags=re.IGNORECASE)
    
    # 处理单个字母间的空格，支持大小写字母
    prediction = re.sub(r"^([a-zA-Z]) ([a-zA-Z][+ ])", r"\1+\2", prediction)
    prediction = re.sub(r"[ +]([a-zA-Z]) ([a-zA-Z]) ", r" \1+\2 ", prediction)
    ...
    # 处理以特定标记开头的情况
    prediction = re.sub(r"([ +]sch) ([a-zA-Z][ +])", r"\1+\2", prediction, flags=re.IGNORECASE)
    prediction = re.sub(r"([ +]nn) ([a-zA-Z][ +])", r"\1+\2", prediction, flags=re.IGNORECASE)
    
    # 删除多余空格
    prediction = re.sub(r" +", " ", prediction)
    
    # 删除重复单词，支持大小写
    prediction = re.sub(r"(?<![\w-])(\b[a-zA-Z]+(?![\w-])) \1(?![\w-])", r"\1", prediction, flags=re.IGNORECASE)
    
    # 移除额外的空格
    prediction = re.sub(r" +", " ", prediction)
    
    # 使用 groupby 去重并确保清理结果干净
    prediction = " ".join(
        " ".join(i[0] for i in groupby(prediction.split(" "))).split()
    )
    prediction = prediction.strip()
...
def clean_csl(prediction):
    # 去除首尾空格
    prediction = prediction.strip()
    # 删除多余空格
    prediction = re.sub(r" +", " ", prediction)
    # 删除连续重复单词，支持大小写
    prediction = re.sub(r"(?<![\w-])(\b[a-zA-Z]+(?![\w-])) \1(?![\w-])", r"\1", prediction)
    # 使用 groupby 去重并确保清理结果干净
    prediction = " ".join(
        " ".join(i[0] for i in groupby(prediction.split(" "))).split()
    )
